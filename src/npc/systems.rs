...
pub fn setup(asset_server: Res<AssetServer>, mut commands: Commands) {
    ...
    commands
        .spawn(SceneBundle {
            scene: asset_server.load("models/animated/npc_emo.gltf#Scene0"),
            ..default()
        })
...
pub fn load_assets(_my_assets: Res<MyAssets>, mut commands: Commands) {
    ...
    // animations.0.iter().for_each(|animation| {
    //     println!("Animation: {:?}", animation);
    // });
    // _my_assets.npc_emo.animations.iter().for_each(|animation| {
    //     commands.insert_resource(animation.clone());
    // });
    //
    //
}
